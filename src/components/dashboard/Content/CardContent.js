...
import { Card, CardBody, Button, Row } from 'shards-react'
...
class Content extends React.Component {
  ...
  render() {
    ...
    return articles.map((article, idx) => (
      <Card className="article-number card-style" key={idx}>
        <CardBody className="p-0">
          <div key={idx} className="blog-comments__item d-flex p-3">
            <Row>
              <div className="blog-comments__avatar mr-3 col-md-5">
                <img
                  src={require('./../../../assets/images/avatars/0.jpg')}
                  alt={article.author}
                />
              </div>
              <div className="blog-comments__meta text-mutes col-md-7">
                <p className="text-secondary name row">{article.author}</p>
                <p className="text-secondary email row">{article.email}</p>
                <span className="text-mutes role row"> {article.jobTitle}</span>
              </div>
              <div className="blog-comments__meta text-mutes col-md-12">
                <p className="m-0 mb-2 text-muted title">{article.title}</p>
              </div>
              <div className="blog-comments__meta text-mutes col-md-12">
                <p className="m-0 mb-2 text-muted description">
                  {article.desc.substring(0, 99)}
                </p>
              </div>
              <div className="col-md-5">
                <Button onClick={() => this.onDeleteArticle(article)}>
                  Delete
                </Button>
              </div>
